...
void HESP_voidInit(void)
{
	...
	/* Loop till receive OK command from wifi */
	...
	/* Local_u8ValidFlag equal 1, set to zero again */
	Local_u8ValidFlag = 0;
	...
	while(Local_u8ValidFlag == 0)
	{
		...
		/*	Check wifi response whether it's OK or not */
...
u8 u8ValidateCmd(u32 Copy_u32TimeOut)
{
	...
	/* Loop to assure that MCU is not receiving any more data from the ESP */
	while (Temp_char != 255)
	{
...
void HESP_voidConnectToRouter(char* Copy_u8StrSSID, char* Copy_u8StrPassword)
{
	...
	/* Concatenate strings=: "AT+CWJAP_CUR="Copy_u8StrSSID","Copy_u8StrPassword"" */
	...
	/* Hold till the wifi module return OK */
	while(Local_u8ValidFlag == 0)
	{
...
void HESP_voidConnectToServer(char* Copy_u8StrMode, char* Copy_u8StrIP, char* Copy_u8StrPort)
{
	...
	/* Concatenate strings: "AT+CIPSTART="Copy_u8StrMode","Copy_u8StrIP",Copy_u8StrPort"
